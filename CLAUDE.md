...
- **Deploy pipeline (git-connected):** the Vercel project is linked to
  `github.com/Rassdor8364/Praevion`; every push to `main` builds and deploys production,
  other branches get preview deployments. `vercel.json` pins the build settings
  (`installCommand: npm install`) — this deliberately OVERRIDES a legacy dashboard
  installCommand that used to bootstrap source from a Supabase-hosted tarball; do not
  remove those keys or the dashboard override resurrects v0.4 source. `vercel.json` also
  schedules the learning-loop crons (daily snapshot + settle; raise cadence on Pro).
  The old `deploy-artifacts` bucket bundles (v1–v4) are historical and can be deleted.
- **Source of truth:** this git repository. The original `praevion-repo-full.tar.gz`
  delivery has been imported wholesale (migrations, tests, scripts included).
